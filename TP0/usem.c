...
1
0
/* Valor inicial de todos los semáforos */
/* Índice del primer semáforo
*/
abrir_sem(int *sid, key_t clave);
crear_sem(int *sid, key_t clave, int idx);
bloquear_sem(int clave, int idx);
zero_espera_sem(int clave, int idx);
desbloquear_sem(int clave, int idx);
remover_sem(int sid);
uso(void);
...
\n");
fprintf(stderr, "
(t)oma recurso compartido
\n");
fprintf(stderr, "
(e)spera IPC de valor cero
\n");
fprintf(stderr, "
(b)orrar
\n");
fprintf(stderr, "
(a)gregar <PATH-DB> <LINE>
\n");
exit(1);
}
/**************************** fín de usem.c *********************************/